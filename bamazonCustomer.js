...
connection.connect(function(err) {
    ...
    new Promise(function(resolve, reject) {
        connection.query('SELECT * FROM products', function(err, res) {
            if (err) reject(err);
            resolve(res);
            console.log('Welcome to Bamazon! We have the following products:')
        });
...
function enterStore() {
    inquirer.prompt([{
        name: 'entrance',
        message: 'Do you want to see our available products',
        type: 'list',
        choices: ['Yes', 'No']
        ...
        if (answer.entrance === 'Yes') {
            ...
            // Exist CLI if No
            console.log('We hope to see you back! --Bamazon');
            connection.destroy();
            return;
        }
    });
...
function menu() {
    return inquirer.prompt([{
        name: 'item',
        message: 'Insert the item number to buy.',
        type: 'input',
